...
for (sub in 1:max(exp2_dat$subject)) {
  df <- NULL
  for (g in search_space) {
    sub_df <- merge(exp2_dat %>% filter(subject == sub), gamma_grid %>% filter(gamma == g))
    df <- rbind(df, sub_df)
  }
  df <- df %>% 
    group_by(gamma) %>% 
...
counts <- table(exp2_dat$team, exp2_dat$target)
...
write.table(model_evidence, 'exp2_model_evidence.csv', sep = ",", col.names = F, row.names = F)
...
pxp <- read.csv('exp2_protected_exceedance_probabilities.csv', header = T)
pxp$planning
...
dat_endpoint <- exp2_dat %>% 
  filter(round == 3) %>% 
  group_by(scenario, s1, s2) %>% 
  dplyr::summarize(n = n()) %>% 
  mutate(model = 'data')

p1 <- individual(dat_endpoint, 1) + labs(x = 'Weaker agent\'s strength', y = 'Stronger agent\'s strength')
