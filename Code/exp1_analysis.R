...
exp1_dat <- read.csv('./../Data/exp1.csv', header = T, stringsAsFactors = T) %>% 
  select(-c(expIndex, weak_strength, strong_strength, target)) %>% 
  filter(round != 0) %>% 
  mutate(round_combination = paste0(agent_selected, weight_selected)) %>% 
...
for (sub in 1:max(exp1_dat$subject)) {
  df <- NULL
  for (g in search_space) {
    sub_df <- merge(exp1_dat %>% filter(subject == sub), gamma_grid %>% filter(gamma == g))
    df <- rbind(df, sub_df)
  }
  df <- df %>% 
    group_by(gamma) %>% 
...
exp1_dat <- read.csv('./../Data/exp1.csv', header = T, stringsAsFactors = T) %>% 
  select(-c(expIndex, weight_selected, agent_selected)) %>% 
...
p1 <- plot_2d(data_sim, 1) + labs(x = NULL, y = 'Stronger agent\'s strength') + theme(legend.position = 'none')
...
exp1_longer <- exp1_dat %>% 
  pivot_longer(c(s1, s2), names_to = 'agent', values_to = 'strength') %>% 
  mutate(agent = substr(agent, 2, 2))

dat_summary <- exp1_longer %>%
...
write.table(model_evidence, 'exp1_model_evidence.csv', sep = ",", col.names = F, row.names = F)
...
pxp <- read.csv('exp1_protected_exceedance_probabilities.csv', header = T)
pxp$planning
...
p1 <- individual(dat_endpoint, 1) + labs(x = NULL, y = 'Stronger agent\'s strength')
p2 <- individual(dat_endpoint, 2) + labs(x = NULL, y = NULL)
